Shader "Custom/LegoBrickTriplanarCheckerboard"
{
    Properties
    {
        _ColorLight     ("Color Light", Color) = (0.9, 0.9, 0.9, 1)
        _ColorDark      ("Color Dark",  Color) = (0.3, 0.3, 0.3, 1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 CheckerboardColor(float3 posWS, float tileSize)
            {
                float2 tile = floor(posWS.xz / tileSize);
                float  checker = fmod(abs(tile.x + tile.y), 2.0);
                return checker < 1.0 ? _ColorLight.rgb : _ColorDark.rgb;
            }
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                surface.albedo     = CheckerboardColor(input.positionWS, _TileSize);
                surface.alpha      = 1.0;
